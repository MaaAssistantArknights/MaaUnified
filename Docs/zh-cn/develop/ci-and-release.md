...
- Linux / macOS 托管应用和依赖在 `staging/bin/`
- MaaCore runtime、原生库和 `resource/` 在 `staging/` 根目录
- Windows 根目录入口是 `staging/MAAUnified.exe`
- Linux 根目录保留 `staging/MAAUnified` 和 `staging/MAAUnified.sh`
