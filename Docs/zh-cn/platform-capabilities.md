...
| --- | --- | --- | --- | --- |
| 托盘 | 原生通知区域图标 | Avalonia 托盘或窗口内菜单；AppKit `NSStatusItem` 可显式启用验证 | Avalonia 托盘能力可用时启用 | Avalonia 托盘或窗口内菜单 |
...
可通过环境变量 `MAA_PLATFORM_FORCE_FALLBACK=1` 强制使用降级路径，便于测试托盘、通知、热键、Overlay 等能力的 fallback 行为。macOS AppKit `NSStatusItem` 托盘 provider 当前需要显式设置 `MAA_PLATFORM_ENABLE_MAC_STATUS_ITEM=1` 才会参与选择，默认使用稳定 fallback 路径。
...
海外源/GitHub 软件更新代码支持 Windows、Linux、macOS 的 MAAUnified 发布包；当前实际发布 Linux 包。Linux 桌面便携 `.zip` 会下载到 `update-packages/`，并在重启后应用；应用时保留 `config/`、`data/`、`cache/`、`debug/` 等用户目录。Beta 通道和 Stable 通道都发布 MAAUnified Linux 包。Windows `.zip` 和 macOS `.dmg` 的更新处理保留在代码中，但对应包暂不上传到 release。

Mirror酱目前仅作为资源更新源使用。选择 Mirror酱检查软件更新时，程序会提示 MAAUnified 暂未支持 Mirror酱软件更新，并引导切换到海外源/GitHub。
## 托盘与通知

Windows 优先使用原生通知区域图标。macOS 默认使用 Avalonia 托盘能力或窗口内菜单；AppKit `NSStatusItem` 状态栏项作为显式启用的验证路径保留，启用后失败会先尝试 Avalonia 托盘能力，再降级到窗口内菜单。Linux 在 Avalonia 托盘能力可用时启用系统托盘；不可用时，托盘菜单会降级到窗口内菜单。托盘菜单仍应提供开始、停止、显示窗口、隐藏托盘、切换 Overlay、重启和退出等命令。

通知优先使用系统桌面通知。macOS 托盘 `ShowAsync` 不发送 AppKit 气泡通知，仍通过通知服务 fallback 记录结果。若系统通知不可用、权限不足或发送失败，程序会回退到应用内通知或命令式通知服务。测试时应关注 `debug/avalonia-platform-events.log` 中的 provider、operationId、usedFallback 和 errorCode。
