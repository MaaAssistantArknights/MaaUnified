...
| 全局热键 | SharpHook 原生热键，失败时回退 | SharpHook 原生热键，失败时回退 | Wayland 优先 xdg-desktop-portal，其他场景可用 SharpHook | 窗口内热键 |
| 自启动 | HKCU Run 注册表项 | LaunchAgents plist | XDG autostart desktop 文件 | 失败时提示，不阻塞启动 |
| Overlay | Win32 窗口附着 | 预览与日志 | X11 目标发现，附着失败时预览与日志 | preview-and-log |
...
macOS 发布包优先使用 Developer ID 签名和 Apple notarization。签名材料缺失时，CI 会输出 warning 并降级生成 ad-hoc signed `.dmg`；该包未公证，用户可能需要在“隐私与安全性”中手动允许，或确认来源后执行 `xattr -dr com.apple.quarantine /Applications/MAAUnified.app`。
...
macOS 当前按预览与日志模式处理。测试者应确认功能入口可见、提示明确、降级后主流程仍可继续。
