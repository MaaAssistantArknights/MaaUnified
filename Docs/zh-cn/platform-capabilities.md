...
海外源/GitHub 软件更新代码支持 Windows、Linux、macOS 的 MAAUnified 发布包；当前实际发布范围只开放 Beta Linux。Linux 桌面便携 `.zip` 会下载到 `update-packages/`，并在重启后应用；应用时保留 `config/`、`data/`、`cache/`、`debug/` 等用户目录。Stable 通道暂不发布 MAAUnified 包。Windows `.zip` 和 macOS `.dmg` 的更新处理保留在代码中，但对应包暂不上传到 release。
