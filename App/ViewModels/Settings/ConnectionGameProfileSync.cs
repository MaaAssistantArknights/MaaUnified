...
internal static class ConnectionGameProfileSync
{
        ...
        bool tolerateMissing = true)
    {
        ...
        var normalizedConnectConfig = NormalizeStoredConnectConfig(storedConnectConfig);
        var fallbackConnectAddress = tolerateMissing
            ? state.ConnectAddress
            : PlayCoverConnectConfigResolver.ResolveDefaultConnectAddress(normalizedConnectConfig, DefaultConnectAddress);
        state.ConnectAddress = ReadProfileStringWithAliases(
            profile,
            fallbackConnectAddress,
            ConnectAddressKey,
            ConnectAddressLegacyKey);
        state.ConnectConfig = normalizedConnectConfig;
    ...
    private static string NormalizeStoredConnectConfig(string connectConfig)
        => NormalizeStoredConnectConfigForPlatform(connectConfig, OperatingSystem.IsMacOS());

    internal static string NormalizeStoredConnectConfigForPlatform(string connectConfig, bool isMacOS)
    {
        var normalized = PlayCoverConnectConfigResolver.IsPlayCoverConnectConfig(connectConfig)
            && !string.Equals(connectConfig, "MacPlayTools", StringComparison.OrdinalIgnoreCase)
            ? "MacPlayTools"
            : connectConfig;

        return SettingsOptionCatalog.IsConnectConfigAvailableForPlatform(normalized, isMacOS)
            ? normalized
